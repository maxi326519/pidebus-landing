...
// Obtén el contenedor dentro de buttonCollapse donde deseas agregar los filtros móviles
const filterCollapseContainer = document.getElementById("filterCollapse");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function filtrarTarjetas() {
    ...
    tarjetasViaje.forEach((tarjeta) => {
      const datosTarjeta = {
        empresa: tarjeta
          .querySelector(".div__card:nth-child(1) .e__tick")
          .textContent.trim(),
        // fechaSalida: tarjeta.querySelector('.date:nth-child(2)').textContent.trim(),
        horaSalida: tarjeta
          .querySelector(".div__card:nth-child(2) .hour")
          .textContent.trim(),
        // fechaLlegada: tarjeta.querySelector('.date:nth-child(6)').textContent.trim(),
        horaLlegada: tarjeta
          .querySelector(".div__card:nth-child(4) .hour")
          .textContent.trim(),
        terminalSalida: tarjeta
          .querySelector(".div__card:nth-child(2) .terminal")
          .textContent.trim(),
        terminalLlegada: tarjeta
          .querySelector(".div__card:nth-child(4) .terminal")
          .textContent.trim(),
        reembolsable: tarjeta
          .querySelector(".div__card:nth-child(6) .reembol")
          .textContent.trim(),
        tipoAsiento: tarjeta
          .querySelector(".div__card:nth-child(3) .cama")
          .textContent.trim(),
      };
      ...
      for (const filtro in filtrosSeleccionados) {
        if (filtrosSeleccionados.hasOwnProperty(filtro)) {
          ...
          if (filtro === "reembolsable") {
            ...
            if (
              !valoresFiltro.includes(datosTarjeta.reembolsable.toLowerCase())
            ) {
